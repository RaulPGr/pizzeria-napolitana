...
export default function OrdersClient() {
  ...
  // Auto-actualización: Realtime + polling de respaldo
  useEffect(() => {
    let t: number | undefined;
    const schedule = () => {
      if (t) window.clearTimeout(t);
      t = window.setTimeout(() => void reload(), 300);
    };
    ...
    return () => { if (t) window.clearTimeout(t); window.clearInterval(iv); supabase.removeChannel(channel); };
  }, []);
  ...
  async function reload() {
    ...
    try {
      ...
      if (json.ok && json.data) setOrders(json.data); else setOrders([]);
    } catch {
      setOrders([]);
  ...
  async function changeStatus(order: OrderRow, next: OrderStatus) {
    ...
    try {
      const r = await fetch(`/api/orders/${order.id}/status`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: next }) });
  ...
  async function toggleDetail(order: OrderRow) {
    const id = order.id;
    if (openDetailId === id) { setOpenDetailId(null); return; }
    setOpenDetailId(id);
    ...
    } catch { alert("No se pudo cargar el detalle del pedido"); }
          ...
          <div className="px-4 py-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(["pending", "confirmed", "preparing", "ready", "delivered", "cancelled"] as OrderStatus[]).map((s) => {
                ...
                return (
                  <button key={s} onClick={() => void changeStatus(o, s)} className={`rounded-full border px-3 py-1 text-sm ${active ? "bg-black text-white" : "bg-gray-100"}`}>
                    {estadoEtiqueta[s]}
                  </button>
                );
              })}
            ...
            <div className="mt-3">
              <button onClick={() => void toggleDetail(o)} className="text-sm font-medium underline underline-offset-2">{openDetailId === o.id ? "Ocultar detalle" : "Ver detalle"}</button>
              {openDetailId === o.id && (
                <div className="mt-3 overflow-hidden rounded-md border">
                  <table className="w-full text-left text-sm"><thead className="bg-gray-50"><tr><th className="px-3 py-2">Cant.</th><th className="px-3 py-2">Producto</th><th className="px-3 py-2">P. unitario</th><th className="px-3 py-2">Importe</th></tr></thead><tbody>
                    {detailLoading[o.id] && (<tr><td colSpan={4} className="px-3 py-3 text-center text-gray-500">Cargando detalle…</td></tr>)}
                    {!detailLoading[o.id] && (detailCache[o.id] || []).map((it) => (
                      <tr key={it.id} className="border-t"><td className="px-3 py-2">{it.quantity}</td><td className="px-3 py-2">{it.name}</td><td className="px-3 py-2">{eur(it.unit_price_cents)}</td><td className="px-3 py-2">{eur(it.line_total_cents)}</td></tr>
                    ))}
                    {!detailLoading[o.id] && detailCache[o.id] && (<tr className="border-t bg-gray-50 font-medium"><td className="px-3 py-2" colSpan={3}>Total</td><td className="px-3 py-2">{eur(o.total_cents)}</td></tr>)}
                  </tbody></table>
                </div>
              )}
            </div>
            ...
            {historico.map((o) => (
              <div key={o.id} className="mb-3 overflow-hidden rounded-md border">
                ...
                <div className="px-3 py-2 text-sm text-gray-700">
                  ...
                  <div className="mt-2">
                    <button onClick={() => void toggleDetail(o)} className="text-xs font-medium underline underline-offset-2">{openDetailId === o.id ? "Ocultar detalle" : "Ver detalle"}</button>
                    {openDetailId === o.id && (
